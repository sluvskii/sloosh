const getData = () => {
  ...
  const titleId = $('.b-userset__fav_holder_data')[0].getAttribute('data-post_id');

  const userId = $('#member_user_id')[0].getAttribute('value');
...
const updateTime = (timer) => {
  if (CDNPlayer.api('playing')) {
    window.postMessage({ action: 'update', data: {
      ...getData(),
      time: CDNPlayer.api('time'),
